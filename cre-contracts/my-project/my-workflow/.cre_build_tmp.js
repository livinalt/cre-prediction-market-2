...
function onHttpTrigger(runtime2, payload) {
  ...
  try {
    ...
    runtime2.log(`[Step 1] Question: "${inputData.question}"`);
    ...
    runtime2.log(`[Step 2] Contract: ${evmConfig.marketAddress}`);
    ...
    runtime2.log("[Step 3] Generating report...");
    ...
    if (writeResult.txStatus === TxStatus.SUCCESS) {
      ...
      runtime2.log(`[Step 5] ✓ Market created: ${txHash}`);
      return txHash;
    }
...
function onLogTrigger(runtime2, log) {
  ...
  try {
    ...
    runtime2.log(`[Step 1] Market #${marketId}: "${question}"`);
    const evmConfig = runtime2.config.evms[0];
    const network248 = getNetwork({
      chainFamily: "evm",
      chainSelectorName: evmConfig.chainSelectorName,
    ...
    if (writeResult.txStatus === TxStatus.SUCCESS) {
      const txHash = bytesToHex(writeResult.txHash || new Uint8Array(32));
      runtime2.log(`[Step 4] ✓ Settled: ${txHash}`);
      runtime2.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
...
var initWorkflow = (config) => {
  ...
  const evmClient = new cre.capabilities.EVMClient(network248.chainSelector.selector);
  const eventSigHash = keccak2562(toBytes3("SettlementRequested(uint256,string)"));
  const evmLogTrigger = evmClient.logTrigger({
    addresses: [hexToBase642(evmConfig.marketAddress)],
    topics: [
      { values: [hexToBase642(eventSigHash)] },
      {},
      {},
      {}
    ]
